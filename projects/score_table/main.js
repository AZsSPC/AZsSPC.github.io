let tables = {
	...
	test2: {
		...
		scores: {
			...
			mana: {name: 'Mana', min: 0, max: 99, val: 63, col: 'aqua'},
		}
	}
}
...
function draw() {
	...
	for (let t_id of Object.keys(tables)) {
		...
		str_b += '<table class="group" id="' + t_id + '"><caption>' +
			'<button is="az-button" color="red" onclick="delete_table(\'' + t_id + '\')">del</button>' + t.name +
			'<button is="az-button" color="green" onclick="new_score(\'' + t_id + '\')">add</button>' + '</caption>'
		for (let s_id of Object.keys(t.scores)) {
			...
			<tr title="${t_id}" class="numintr" ${s.col ?`style="--borc:${s.col}" ` :''}>
				<td>
					<label for="${t_id}-${s_id}">${s.name}
						<button class="r-btn" onclick="delete_score('${t_id}','${s_id}')">-</button>
					</label>
				</td>
				<td class="inp_num">
					<label for="${t_id}-${s_id}" max="${s.max}" min="${s.min}" val="${s.val}">
						<input onchange="tables.${t_id}.scores.${s_id}.val=((this.value*100)|0)/100" type="number" 
						id="${t_id}-${s_id}" max="${s.max}" min="${s.min}" value="${s.val}">
					</label>
				</td>
			</tr>`
...
function new_score_template() {
	...
	<label is="az-input-button" color="gray">Name <input type="text" id="c_name" name="c_name" oninput="nextElementSibling.value=value" pattern=".+" placeholder="name"></label>
	<label is="az-input-button" color="gray">ID <input type="text" id="c_id" name="" pattern="[a-z_]+" title="required a-z and _" placeholder="id"></label>
	<label is="az-input-button" color="gray">Min <input type="number" id="c_min" name="c_min" value="0"></label>
	<label is="az-input-button" color="gray">Max <input type="number" id="c_max" name="c_max" value="100"></label>
	<label is="az-input-button" color="gray">Value <input type="number" id="c_val" name="c_val" value="0"></label>
	<label is="az-input-button" color="gray">Color <input type="color" id="c_c" name="c_c" oninput="parentNode.style.background=value"></label>
	<button is="az-button" color="purple" onclick="create_score_template()">Save score template</button>`

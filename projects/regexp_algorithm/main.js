	...
	while (AZ.loop.value) {
		if (iterator >= runable.length) {
			...
			if (s.match(pattern)) {
				...
				markQuery(iterator)
				iterator = 0
				if (rule.exit) AZ.loop.value = false
				await new Promise(res => setTimeout(res, AZ.loop.timeout))
			} else {
				iterator++
			}
		}
...
function addRule(from = '', to = '', exit = false, off = false) {
    ...
    if (off) li.setAttribute('off', '');

    const div = document.createElement('div');

    const fromSpan = document.createElement('span');
    fromSpan.className = 'from';
    fromSpan.setAttribute('contenteditable', 'true');
    fromSpan.oninput = () => redrawFrom(fromSpan);
    fromSpan.innerHTML = retextFrom(from);

    const exitSpan = document.createElement('span');
    exitSpan.className = 'exit';
    exitSpan.onclick = () => switchExit(exitSpan);
    if (exit) exitSpan.setAttribute('t', '');
    exitSpan.innerHTML = exit ? TERMINATION : STEP;

    const toSpan = document.createElement('span');
    toSpan.className = 'to';
    toSpan.setAttribute('contenteditable', 'true');
    toSpan.oninput = () => redrawTo(toSpan);
    toSpan.innerHTML = to;

    const delSpan = document.createElement('span');
    delSpan.className = 'del';
    delSpan.onclick = () => nma_table.removeChild(li);
    delSpan.innerHTML = '-';

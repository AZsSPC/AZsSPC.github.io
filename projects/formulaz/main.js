...
function reload_data(url = rel_url) {
    ...
    xhr.onreadystatechange = function () {
        ...
        f_buttons.innerHTML = '<input placeholder="Find..." id="find_labe" onchange="find(this.value)" oninput="if(this.value.length>2) find(this.value)" type="text">';
        for (let b of DATA.buttons) f_buttons.innerHTML += ' <button onclick="find(\'' + b.pattern + '\')">' + b.name + '</button>';

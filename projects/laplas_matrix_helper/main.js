...
function userStepMatrix() {
	...
	for (let h = 0; h < height; h++) for (let w = 0; w < width; w++)
		document.getElementById('me_i' + w + '_j' + h).innerText = currentMatrix[h][w]
...
function generateSolution() {
	...
	for (let i = 0; i < commands.length; i++)
		TeX += '\\overset{\\mbox{' + commands[i] + '}}{\\sim}' + matrixTeX(temp = stepMatrix(temp, parseCommand(commands[i]))) + '\n'
	solutionTeX.innerText = '$$' + TeX + '$$'
	console.log(TeX)
	MathJax.Hub.Queue(['Typeset', MathJax.Hub])
	let filename = prompt('What do you want to name the file?', 'matrix_sol_by_az')
	if (filename) {
		create_file_and_download(filename + '.tex', TeX)
	}
}
